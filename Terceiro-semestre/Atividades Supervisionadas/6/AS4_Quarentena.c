/*Síntese
	Objetivo: "Lista de produtos feita através de uma LDE com descritor, mostrando percentual de vogal e consoantes que tem na descrição".
...
void informar(int , int , struct tDescritor );
void comparar (int *, int *, struct tDescritor );
void liberar(struct tDescritor *);

int main (void){
	int op, pesqCod, qtdV = 0, qtdC=0;
	...
	do{
		...
		switch(op){
			...
			case 6:
				informar(qtdV, qtdC, lista);
			break;
		}
		comparar(&qtdV, &qtdC, lista);
	}while(op!=0);
...
void informar(int qtdV, int qtdC, struct tDescritor list){
	int mediaV = 0, mediaC = 0;
	
	mediaV = (qtdV*100)/list.qtd;
	mediaC = (qtdC*100)/list.qtd;
	printf("Quantidade de produtos:%d\n", list.qtd);
	printf("Quantidade percentual de descrições que iniciam com vogais:%d\n", mediaV);
	printf("Quantidade percentual de descrições que iniciam com consoantes:%d\n", mediaC);
	getch();
}

void comparar (int *qtdV, int *qtdC, struct tDescritor list){
	*qtdV = 0;
	*qtdC = 0;
	struct tNo *p = list.ini;
	char letra;
	...
	while(p!= NULL){
		letra = toupper(p->dado.descricao[0]);
